...
def get_db():
    ...
    if db is None:
        ...
        wtimeout=2500,
        # connection atlas memerlukan SSL, jika error SSL maka gunakan c=syntax dibawah
...
def login_user(email, jwt):
    ...
    try:
        ...
        db.sessions.update_one(
            { "user_id": get_user(email).get('email') },
            { "$set": { "jwt": jwt } },
            upsert=True
        )
        return {"success": True}
...
def logout_user(email):
    ...
    try:
        ...
        db.sessions.delete_one({ "user_id": get_user(email).get('email')  })
        return {"success": True}
    except Exception as e:
        return {"error": e}
...
def get_user_session(email):
    ...
    try:
        ...
        return db.sessions.find_one({ "user_id": get_user(email).get('email')  })
    except Exception as e:
        return {"error": e}
...
def delete_user(email):
    ...
    try:
        ...
        db.sessions.delete_one({ "user_id": get_user(email) })
